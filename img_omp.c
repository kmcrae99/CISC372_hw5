...
//Returns: Nothing
void omp_convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    int row,pix,bit,span;
    span=srcImage->bpp*srcImage->bpp;
    int num_threads = omp_get_num_threads();
    int count = srcImage->height / num_threads;
    int my_rank = omp_get_thread_num();
    int last;
    row = my_rank*count;
    if(my_rank == num_threads-1){
	last = srcImage->height;
    } else {
    	last = row + count;
    }
...
int main(int argc,char** argv){
    ...
    omp_convolute(&srcImage,&destImage,algorithms[type]);
